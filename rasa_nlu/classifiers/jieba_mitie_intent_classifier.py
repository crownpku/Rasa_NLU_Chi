...
class JiebaMitieIntentClassifier(Component):

    name = "intent_classifier_jieba_mitie"
    ...
    def train(self, training_data, mitie_file, num_threads):
        ...
        for example in training_data.intent_examples:
            ...
            tokens = [w.encode('utf-8') for w in jieba.lcut(example["text"])]
    ...
    @classmethod
    def load(cls, model_dir, intent_classifier_jieba_mitie):
        # type: (Text, Text) -> MitieIntentClassifier
        import mitie

        if model_dir and intent_classifier_jieba_mitie:
            classifier_file = os.path.join(model_dir, intent_classifier_jieba_mitie)
            classifier = mitie.text_categorizer(classifier_file)
            return JiebaMitieIntentClassifier(classifier)
        else:
            return JiebaMitieIntentClassifier()
    ...
    def persist(self, model_dir):
        ...
        if self.clf:
            ...
            return {"intent_classifier_jieba_mitie": "intent_classifier.dat"}
        else:
            return {"intent_classifier_jieba_mitie": None}
